...
from expe.config.different_grid import RUN_NAMES_GRID
...
def main(cfg: DictConfig) -> None:
    ...
    inr_run_name = RUN_NAMES_GRID[sub_from][sub_tr]["coral"]["inr"]
    dyn_run_name = RUN_NAMES_GRID[sub_from][sub_tr]["coral"]["dyn"]
    dino_run_name = RUN_NAMES_GRID[sub_from][sub_tr]["dino"]
    ...
    times_train_coral = torch.zeros(4).cuda()
    times_train_dino = torch.zeros(4).cuda()
    losses_test = torch.zeros((n_baselines, 4, 2)).cuda()
    times_test_coral = torch.zeros(4).cuda()
    times_test_dino = torch.zeros(4).cuda()
    ...
    for up, loader in enumerate([train_loader_ext, train_loader_up1, train_loader_up4, train_loader_up16]):
        ...
        for bidx, batch in enumerate(loader):
            ...
            if inr_run_name is not None:
                ...
                tac = time.time()
                losses_train[i, up, 0] += criterion(pred_coral[..., :sequence_length_in],
                                                    images[..., :sequence_length_in]) * n_samples
                losses_train[i, up, 1] += criterion(pred_coral[..., sequence_length_in:sequence_length_in+sequence_length_out],
                                                    images[..., sequence_length_in:sequence_length_in+sequence_length_out]) * n_samples
                times_train_coral[up] += (tac-tic)
                i += 1
            if dino_run_name is not None:
                ...
                losses_train[i, up, 1] += criterion(pred_coral[..., sequence_length_in:sequence_length_in+sequence_length_out],
                                                    images[..., sequence_length_in:sequence_length_in+sequence_length_out]) * n_samples
                times_train_dino[up] += (tac-tic)
                i += 1
            # break
        i = 0
        if inr_run_name is not None:
            print(
                f"CORAL train up sampling rate {upsamplings[up]} in-t: {losses_train[i, up, 0]/ n_seq_train}")
            print(
                f"CORAL train up sampling rate {upsamplings[up]} out-t: {losses_train[i, up, 1]/ n_seq_train}")
            print(
                f"CORAL train up sampling rate {upsamplings[up]} inference time for batch_size {batch_size}: {times_train_coral[up] / n_seq_train}")
            i += 1
        if dino_run_name is not None:
            print(
                f"DINO train up sampling rate {upsamplings[up]} in-t: {losses_train[i, up, 0]/ n_seq_train}")
            print(
                f"DINO train up sampling rate {upsamplings[up]} out-t: {losses_train[i, up, 1]/ n_seq_train}")
            print(
                f"DINO train up sampling rate {upsamplings[up]} inference time for batch_size {batch_size}: {times_train_dino[up]/ n_seq_train}")

    losses_train = losses_train.cpu() / n_seq_train
    times_train_coral = times_train_coral.cpu().numpy().astype(np.float64) / n_seq_train
    times_train_dino = times_train_dino.cpu().numpy().astype(np.float64) / n_seq_train
    print(losses_train)

    # if sub_tr == 0.05:
    #     TIMES_UP["coral"]["train"][f'{upsamplings[0]}'] = times_train_coral[0]
    #     TIMES_UP["coral"]["train"][f'{upsamplings[1]}'] = times_train_coral[1]
    #     TIMES_UP["coral"]["train"][f'{upsamplings[2]}'] = times_train_coral[2]
    #     TIMES_UP["coral"]["train"][f'{upsamplings[3]}'] = times_train_coral[3]
    #     TIMES_UP["dino"]["train"][f'{upsamplings[0]}'] = times_train_dino[0]
    #     TIMES_UP["dino"]["train"][f'{upsamplings[1]}'] = times_train_dino[1]
    #     TIMES_UP["dino"]["train"][f'{upsamplings[2]}'] = times_train_dino[2]
    #     TIMES_UP["dino"]["train"][f'{upsamplings[3]}'] = times_train_dino[3]
    # TIMES["coral"]["train"][f'{sub_tr}'] = times_train_coral[0]
    # TIMES["dino"]["train"][f'{sub_tr}'] = times_train_dino[0]
    ...
    for up, loader in enumerate([test_loader_ext, test_loader_up1, test_loader_up4, test_loader_up16]):
        ...
        for bidx, batch in enumerate(loader):
            ...
            if inr_run_name is not None:
                ...
                pred_coral = forward_coral(
                    inr, dyn, batch, inner_steps, alpha, True, timestamps_ext, z_mean, z_std, dataset_name)
                tac = time.time()
                losses_test[i, up, 0] += criterion(pred_coral[..., :sequence_length_in],
                                                   images[..., :sequence_length_in]) * n_samples
                losses_test[i, up, 1] += criterion(pred_coral[..., sequence_length_in:sequence_length_in+sequence_length_out],
                                                   images[..., sequence_length_in:sequence_length_in+sequence_length_out]) * n_samples
                times_test_coral[up] += (tac-tic)
                i += 1
            if dino_run_name is not None:
                tic = time.time()
        ...
        if inr_run_name is not None:
            print(
                f"CORAL test up sampling rate {upsamplings[up]} in-t: {losses_test[i, up, 0] / n_seq_test}")
            print(
                f"CORAL test up sampling rate {upsamplings[up]} out-t: {losses_test[i, up, 1] / n_seq_test}")
            print(
                f"CORAL test up sampling rate {upsamplings[up]} inference time for batch_size {batch_size}: {times_test_coral[up] / n_seq_test}")
            i += 1
        if dino_run_name is not None:
            print(
                f"DINO test up sampling rate {upsamplings[up]} in-t: {losses_test[i, up, 0] / n_seq_test} ")
            print(
                f"DINO test up sampling rate {upsamplings[up]} out-t: {losses_test[i, up, 1] / n_seq_test}")
            print(
                f"DINO test up sampling rate {upsamplings[up]} inference time for batch_size {batch_size}: {times_test_dino[up] / n_seq_test}")

    losses_test = losses_test.cpu() / n_seq_test
    times_test_coral = times_test_coral.cpu().numpy().astype(np.float64) / n_seq_test
    times_test_dino = times_test_dino.cpu().numpy().astype(np.float64) / n_seq_test
    print(losses_test)
    # if sub_tr == 0.05:
    #     TIMES_UP["coral"]["test"][f'{upsamplings[0]}'] = times_test_coral[0]
    #     TIMES_UP["coral"]["test"][f'{upsamplings[1]}'] = times_test_coral[1]
    #     TIMES_UP["coral"]["test"][f'{upsamplings[2]}'] = times_test_coral[2]
    #     TIMES_UP["coral"]["test"][f'{upsamplings[3]}'] = times_test_coral[3]
    #     TIMES_UP["dino"]["test"][f'{upsamplings[0]}'] = times_test_dino[0]
    #     TIMES_UP["dino"]["test"][f'{upsamplings[1]}'] = times_test_dino[1]
    #     TIMES_UP["dino"]["test"][f'{upsamplings[2]}'] = times_test_dino[2]
    #     TIMES_UP["dino"]["test"][f'{upsamplings[3]}'] = times_test_dino[3]
    # TIMES["coral"]["test"][f'{sub_tr}'] = times_test_coral[0]
    # TIMES["dino"]["test"][f'{sub_tr}'] = times_test_dino[0]
