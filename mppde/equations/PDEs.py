...
from mppde.common.derivatives import WENO, FDM
...
class CE(PDE):
    ...
    def __init__(self,
        ...
        assert (self.flux_splitting == f'godunov') or (
            self.flux_splitting == f'laxfriedrichs')
    ...
    def WENO_reconstruction(self, t: torch.Tensor, u: torch.Tensor) -> torch.Tensor:
        ...
        if self.flux_splitting == f'godunov':
            dudt = - self.alpha * \
                self.weno.reconstruct_godunov(u_padded_weno, self.dx)

        if self.flux_splitting == f'laxfriedrichs':
            dudt = - self.alpha * \
                self.weno.reconstruct_laxfriedrichs(u_padded_weno, self.dx)
...
class WE(PDE):
    ...
    def chebdx(self, t: np.array, u: np.array, x: np.array, c: float = 1.) -> np.array:
        ...
        if(boundary_condition_left is not boundary_condition_right):
            wt = c ** 2 * self.cheb.solve(v, x, {boundary_condition_left: (
                0, None), boundary_condition_right: (None, 0)}, m=2)
        else:
            wt = c ** 2 * \
                self.cheb.solve(v, x, {boundary_condition_left: (0, 0)}, m=2)
