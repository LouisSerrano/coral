...
class GNN_Layer_Louis(MessagePassing):
    """
    ...
    def __init__(self,
        ...
        """
        super(GNN_Layer_Louis, self).__init__(node_dim=-2, aggr='mean')
...
class MP_PDE_Solver_Louis(torch.nn.Module):
    """
    MP-PDE solver class
    """
    ...
    def __init__(self,
        ...
        """
        super(MP_PDE_Solver_Louis, self).__init__()
        ...
        self.gnn_layers = torch.nn.ModuleList(modules=(GNN_Layer_Louis(
        ...
        self.gnn_layers.append(GNN_Layer_Louis(in_features=self.hidden_features,
                                               hidden_features=self.hidden_features,
                                               out_features=self.hidden_features,
                                               time_window=self.time_window,
                                               input_dim=self.input_dim,
        ...
        self.embedding_mlp = nn.Sequential(
            nn.Linear(self.time_window * self.input_dim + self.pos_dim,
                      self.hidden_features),  # + len(self.eq_variables)
            Swish(),
        ...
        if (self.time_window == 1):
            self.output_mlp = nn.Sequential(nn.Linear(hidden_features, 64), Swish(
            ), nn.Linear(64, self.output_dim*self.time_window))
