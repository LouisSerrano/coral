...
def training_loop(model: torch.nn.Module,
    ...
    for i, (u_super, _, x, idx) in enumerate(loader):
        ...
        steps = [t for t in range(graph_creator.tw,
                                  graph_creator.t_in - graph_creator.tw - (graph_creator.tw * unrolled_graphs) + 1)]
        ...
        if f'{model}' == 'GNN':
            graph = graph_creator.create_graph(
                data, labels, x[:, 0, :, :], variables, random_steps, mode='train').to(device)
            ...
            for _ in range(unrolled_graphs):
                ...
                if f'{model}' == 'GNN':
                    ...
                    graph = graph_creator.create_next_graph(
                        graph, pred, labels, random_steps, mode='train').to(device)
                else:
                    data = model(data)
