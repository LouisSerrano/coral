...
from mppde.common.utils import HDF5Dataset, GraphCreator
from mppde.experiments.models_gnn import MP_PDE_Solver
from mppde.experiments.models_cnn import BaseCNN
from mppde.experiments.train_helper import training_loop, test_timestep_losses, test_unrolled_losses
...
def train(pde: PDE,
    ...
    for i in range(graph_creator.t_res):
        losses = training_loop(model, unrolling, batch_size,
                               optimizer, loader, graph_creator, criterion, device)
        if(i % print_interval == 0):
            print(
                f'Training Loss (progress: {i / graph_creator.t_res:.2f}): {torch.mean(losses)}')
...
def main(args: argparse):
    ...
    if args.experiment == 'E1' or args.experiment == 'E2' or args.experiment == 'E3':
        ...
        assert(base_resolution[1] == 100 or base_resolution[1]
               == 50 or base_resolution[1] == 40)
    elif args.experiment == 'WE1' or args.experiment == 'WE2' or args.experiment == 'WE3':
        pde = WE(device=device)
        assert (base_resolution[0] == 250)
        assert (base_resolution[1] == 100 or base_resolution[1] ==
                50 or base_resolution[1] == 40 or base_resolution[1] == 20)
        if args.model != 'GNN':
            raise Exception(
                "Only MP-PDE Solver is implemented for irregular grids so far.")
    else:
    ...
    try:
        train_dataset = HDF5Dataset(train_string, pde=pde, mode='train',
                                    base_resolution=base_resolution, super_resolution=super_resolution)
    ...
    for epoch in range(args.num_epochs):
        print(f"Epoch {epoch}")
        train(args, pde, epoch, model, optimizer, train_loader,
              graph_creator, criterion, device=device)
        print("Evaluation on validation dataset:")
        val_loss = test(args, pde, model, valid_loader,
                        graph_creator, criterion, device=device)
        if(val_loss < min_val_loss):
            print("Evaluation on test dataset:")
            test_loss = test(args, pde, model, test_loader,
                             graph_creator, criterion, device=device)
            # Save model
