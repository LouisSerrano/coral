...
class MP_PDE_Solver(torch.nn.Module):
    ...
    def __init__(self,
        ...
        self.embedding_mlp = nn.Sequential(
            nn.Linear(self.time_window + 2 +
                      len(self.eq_variables), self.hidden_features),
            Swish(),
    ...
    def forward(self, data: Data) -> torch.Tensor:
        ...
        if "alpha" in self.eq_variables.keys():
            variables = torch.cat(
                (variables, data.alpha / self.eq_variables["alpha"]), -1)
        if "beta" in self.eq_variables.keys():
            variables = torch.cat(
                (variables, data.beta / self.eq_variables["beta"]), -1)
        if "gamma" in self.eq_variables.keys():
            variables = torch.cat(
                (variables, data.gamma / self.eq_variables["gamma"]), -1)
        ...
        if "c" in self.eq_variables.keys():
            variables = torch.cat(
                (variables, data.c / self.eq_variables["c"]), -1)

        # Encoder and processor (message passing)
        print("u.shape, pos_x.shape, variables.shape : ", u.shape, pos_x.shape, variables.shape)
