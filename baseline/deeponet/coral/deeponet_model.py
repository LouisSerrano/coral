...
from deeponet.coral.network import Swish
...
class DeepONet(StructureNN):
    ...
    def __init__(self, branch_dim, trunk_dim, branch_depth=2, trunk_depth=3, width=50, activation='relu', logger=None, input_dataset="navier"):
        ...
        self.logger = logger
        self.is_wave = (input_dataset == "wave")
    ...
    def forward(self, x_branch, x_trunk):
        ...
        if self.is_wave: # TODO : multichannels
...
def AR_forward(deeponet, t, coords, ground_truth, logger=None, is_test=False):
    ...
    for i, t in enumerate(t):
        ...
        # if is_test:
        #     if im.size(3) == 2:
        #         x_branch = x_branch.reshape(im.size(0), 64, 64, im.size(3))[:, :, :]
        #     else:
        #         x_branch = x_branch.reshape(im.size(0), 64, 64)[:, :]
