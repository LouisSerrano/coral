...
load_cfg = input_model["cfg"]
model = ResNet(input_dim=latent_dim_in,
            hidden_dim=load_cfg.model.width,
            output_dim=latent_dim_out,
            depth=load_cfg.model.depth,
            dropout=load_cfg.model.dropout).cuda()
model.eval()
model.load_state_dict(input_model['model'])
...
step_size = 1000
gamma = 0.5
theta = torch.zeros(7, dtype=torch.float, requires_grad=True, device="cuda") #0.001*torch.randn(7, dtype=torch.float, requires_grad=True, device="cuda") #torch.zeros(7, dtype=torch.float, requires_grad=True, device="cuda")
...
for ep in range(epochs):
    ...
    #print('x', x.shape, grid.shape, grid.max(), grid.min())
    ...
    modulations = (modulations - mu_a.cuda())/sigma_a.cuda()
    z_pred = model(modulations)
    out = inr_out.modulated_forward(grid.cuda(), z_pred)
    ...
    a_pred = inr_in.modulated_forward(grid.cuda(), modulations)
    tmp_loss = ((a_pred - x)**2).mean()

    print(ep, t2 - t1, drag.item(), lift.item(), reg.item(), loss.item(), tmp_loss.item())
    ...
    if ep%step_size==0:
        ...
        ax[0].pcolormesh(X, Y, pred, shading='gouraud', rasterized=True)
        ax[1].pcolormesh(X_small, Y_small, pred_small, shading='gouraud', rasterized=True)
        plt.savefig(f"airfoil_small_inverse_design_{ep}.pdf", dpi=50)
